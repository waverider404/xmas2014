...
;( function ( THREE, ns ) {
  ...
  ns.CharacterController.prototype = {
    ...
    events: function () {
      ...
      // 初回のみ、過去状態を作るだけで終わり
      var isFirstUpdate = typeof this.wasGrounded === 'undefined';

      if ( isFirstUpdate ) {

        this.wasGrounded = this.isGrounded;
        this.wasOnSlope  = this.isOnSlope;
        this.wasIdling   = this.isIdling;
      ...
      if ( !this.wasRunning && !this.isRunning && this.isGrounded && !this.isIdling ) {

        this.isIdling = true;
        this.dispatchEvent( { type: 'startIdling' } );

      } else if (
        ( !this.wasRunning && this.isRunning && !this.isJumping && this.isGrounded ) ||
        ( !this.wasGrounded && this.isGrounded && this.isRunning ) ||
        ( this.wasOnSlope && !this.isOnSlope && this.isRunning && this.isGrounded )
      ) {
